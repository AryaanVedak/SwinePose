# ============================================================
# ViTPose-S Stage 1: Frozen backbone, 30 epochs
# Train: train_internal.json (2634 frames, 68 videos, 85% random frame split)
# Val:   val_internal.json   (465 frames,  68 videos, 15% random frame split)
...
# Val evaluator uses internal val (for checkpoint selection during training)
val_evaluator = dict(
    type='CocoMetric',
...
test_evaluator = dict(
    ...
    nms_mode='none',
    outfile_prefix=data_root + '/vitpose_predictions_stage1_test')

# ── Stage 1: Backbone FROZEN, head only, 30 epochs ───────────────────────
optim_wrapper = dict(
...
param_scheduler = [
    ...
    dict(type='MultiStepLR', begin=0, end=30, milestones=[20, 27], gamma=0.1, by_epoch=True)
]
